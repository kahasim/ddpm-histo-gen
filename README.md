...
✅ Fixed: funny enough this was due to creating an initial time embedding `t` as a `torch.tensor` instead of `np.ndarray` when converting from functions to classes, causing the tensor to be added onto the computational graph and gradients backpropogated - the results were generated using the non-class based code.
